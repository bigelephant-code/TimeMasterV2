...
function registerIpc() {
  electron.ipcMain.handle("data:snapshot", () => repo.snapshot());
  electron.ipcMain.handle("settings:get", () => getSettings());
  electron.ipcMain.handle("list:create", (_e, name) => {
    const r = repo.createList(name);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("list:update", (_e, id, patch) => {
    const r = repo.updateList(id, patch);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("list:remove", (_e, id) => {
    const r = repo.removeList(id);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("todo:create", (_e, input) => {
    const r = repo.createTodo(input);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("todo:update", (_e, id, patch) => {
    const r = repo.updateTodo(id, patch);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("todo:toggle", (_e, id) => {
    const r = repo.toggleTodo(id);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("todo:start", (_e, id) => {
    const r = repo.startTodo(id);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("todo:stop", (_e, id) => {
    const r = repo.stopTodo(id);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("todo:remove", (_e, id) => {
    const r = repo.removeTodo(id);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("todo:clearCompleted", (_e, listId) => {
    const r = repo.clearCompleted(listId);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("goal:create", (_e, input) => {
    const r = repo.createGoal(input);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("goal:update", (_e, id, patch) => {
    const r = repo.updateGoal(id, patch);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("goal:addProgress", (_e, id, delta) => {
    const r = repo.addGoalProgress(id, delta);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("goal:remove", (_e, id) => {
    const r = repo.removeGoal(id);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("expense:add", (_e, input) => {
    const r = repo.addExpense(input);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("expense:update", (_e, id, patch) => {
    const r = repo.updateExpense(id, patch);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("expense:remove", (_e, id) => {
    const r = repo.removeExpense(id);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("expense:clearDay", (_e, goalId, date) => {
    const r = repo.clearLedgerDay(goalId, date);
    if (r.ok) pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("expense:exportExcel", async (e, input) => {
  ...
  electron.ipcMain.handle("focus:setDuration", (_e, minutes) => {
    const r = repo.setFocusDuration(minutes);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("focus:start", (_e, minutes) => {
    const r = repo.startFocus(minutes);
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("focus:pause", () => {
    const r = repo.pauseFocus();
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("focus:resume", () => {
    const r = repo.resumeFocus();
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("focus:finish", () => {
    const r = repo.finishFocus();
    pushSnapshot();
    return r;
  });
  electron.ipcMain.handle("focus:cancel", () => {
  ...
  electron.ipcMain.handle("win:minimize", (e) => {
    const win = windowOf(e);
    if (win === getMainWindow()) return returnToWidget();
    win?.minimize();
  });
  electron.ipcMain.handle("win:toggleMaximize", (e) => {
    const win = windowOf(e);
    if (!win) return;
    if (win.isMaximized()) win.unmaximize();
    else win.maximize();
  });
  electron.ipcMain.handle("win:close", (e) => {
    const win = windowOf(e);
    if (win === getMainWindow()) return returnToWidget();
    win?.close();
  });
  electron.ipcMain.handle("win:isMaximized", (e) => !!windowOf(e)?.isMaximized());
  electron.ipcMain.handle("widget:hide", () => {
    patchSettings({ widget: { enabled: false } });
    destroyWidgetWindow();
    pushSettings();
  });
  electron.ipcMain.handle("widget:openMain", (e, target) => {
  ...
  electron.ipcMain.handle("app:openDataDir", () => electron.shell.openPath(electron.app.getPath("userData")));
  electron.ipcMain.handle("app:flush", () => flushNow());
}
...
electron.app.setPath("userData", node_path.join(electron.app.getPath("appData"), "timemaster-v2"));
let tray = null;
