...
感谢你帮助改进时间大师。项目仍处于早期阶段，最有价值的贡献是范围清晰、可复现、保护数据兼容性的 Issue、测试和 Pull Request。
...
- `src/` 是当前 0.1.4 正式版本及后续开发的唯一可编辑构建输入。新功能、修复和安全加固应在这里完成。
...
准备正式产物时，还应为 Windows x64 安装包使用独立版本号、计算最终文件 SHA-256，并明确记录 Authenticode 签名状态。0.1.4 正式安装包未签名；文档定版本身不代表 GitHub Release 附件已经上传。
