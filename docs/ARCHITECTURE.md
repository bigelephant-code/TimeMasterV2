...
应用显式把 Electron `userData` 指向 `%APPDATA%\timemaster-v2\`。主要数据采用 JSON；写入时使用临时文件再替换，并保留恢复副本和无法解析的 `.broken-*` 文件。0.1.3 正式版使用数据模型 v3；0.1.4—0.1.6 Windows x64 正式版使用 v4。0.1.6 安装包未经过 Authenticode 签名；本架构说明不表示该安装包已经上传到 GitHub Release。
...
0.1.3—0.1.6 正式版中，只有天气模块有意访问外部服务；费用分类管理和界面改版不增加联网目的地：
