...
应用显式把 Electron `userData` 指向 `%APPDATA%\timemaster-v2\`。主要数据采用 JSON；写入时使用临时文件再替换，并保留恢复副本和无法解析的 `.broken-*` 文件。0.1.3 正式版使用数据模型 v3；2026-08-12 定版的 0.1.4 Windows x64 正式版使用 v4。0.1.4 安装包未经过 Authenticode 签名；本架构说明不表示该安装包已经上传到 GitHub Release。
...
0.1.3 和 0.1.4 正式版中，只有天气模块有意访问外部服务；费用类别管理不增加联网目的地：
