...
应用显式把 Electron `userData` 指向 `%APPDATA%\timemaster-v2\`。主要数据采用 JSON；写入时使用临时文件再替换，并保留恢复副本和无法解析的 `.broken-*` 文件。0.1.3 正式版使用数据模型 v3；0.1.4—0.1.9 Windows x64 正式版使用 v4。0.1.9 在 v4 待办中增加兼容的可选 `rolloverHistory` 数组，不改变旧数据的版本门禁。0.1.9 安装包未经过 Authenticode 签名；本架构说明不表示该安装包已经上传到 GitHub Release。

主要实体包括：

- `lists`：待办清单；
- `todos`：待办、起止时间、重复、提醒、耗时，以及自动顺延时附着在待办上的只读未完成历史；
...
0.1.3—0.1.9 正式版中，只有天气模块有意访问外部服务；费用分类管理、顺延历史和界面改版不增加联网目的地：
