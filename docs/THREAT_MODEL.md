...
Target: the TimeMaster 0.1.11 formal release, the preserved TimeMaster V2 0.1.3 release artifact, and the opt-in OpenClaw QQ Bot reminder path currently present only in unreleased source on Windows x64
...
- Limit intentional runtime network access to the documented Open-Meteo weather feature and the explicitly enabled, loopback-only OpenClaw reminder bridge.
...
- OpenClaw loopback URL validation, both Hook-token storage boundaries and their Windows ACLs, QQ target/account validation, reminder payload fields, retry/idempotency behavior, or any claim about delivery status;
