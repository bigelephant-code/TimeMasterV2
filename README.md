...
- 应用没有账号体系、广告、会员、分析或遥测上报。天气是 0.1.3—0.1.11 正式版中唯一有意联网的功能；当前未发布源码另增加了默认关闭的 OpenClaw QQ Bot 远程提醒。普通待办、专注、目标和费用数据仍本地优先；只有在用户主动开启远程提醒时，指定的提醒字段才会经本机 OpenClaw 转发。
