...
- 输入“开通速卖通”这类模糊待办后，可让专用 OpenClaw Agent 给出下一步行动、待确认问题、准备材料、分步操作、HTTPS 入口、注意事项和完成后的配套建议。
- “AI 安排今天”先让模型判断顺序和预计时长，再由时间大师本地确定性排程器避开手工时间、午休与已占用时段。模型只生成草案；用户点击“应用安排”后才会批量改动时间，并可按同一批次撤销。
- 功能默认关闭；备注默认不发送。时间大师只向本机 loopback `/v1/responses` 发送当前请求所需的最小任务字段，不把费用、专注记录、目标或整个数据文件交给 Agent。模型仍可能是远程服务，处理方式由用户自己的 OpenClaw 与模型供应商决定。
- Gateway Token 与远程提醒的 Hook token 是两套不同凭据。时间大师侧副本分别使用 Electron `safeStorage` 加密；Responses 的共享 Gateway Token 具有高权限，建议为时间大师使用独立 Gateway 和受限 Agent。
- 配置与安全边界见 [OpenClaw AI 任务教练配置指南](docs/OPENCLAW_AI_TASK_COACH.md)。

### 开发中（尚未发布）：OpenClaw QQ Bot 远程提醒

- 远程提醒默认关闭，启用后可将 QQ Bot 作为主远程通道；时间大师运行时会独立尝试 Windows 本地通知，不会因 OpenClaw 失败或 QQ 限制而跳过本地路径，但系统通知设置仍可能抑制展示。
- 启用时，时间大师只把提醒标题、日期/时间和用户明确选择附带的备注发给本机 loopback 地址上的 OpenClaw。OpenClaw 可能继续调用用户配置的模型和 QQ Bot，其数据处理受对应服务的配置与政策约束。
- 时间大师保存的 Hook token 副本通过 Electron `safeStorage` 单独加密，不写入普通 `settings.json` 或 `data.json`；OpenClaw 仍需在其配置或运行环境中持有同一 token，并应以 Windows ACL 单独保护。`/hooks/agent` 返回 200 只表示 OpenClaw 已受理，不代表 QQ 已送达；QQ 主动消息还可能受目标类型、账号选择和平台交互时限影响。
- 完整的本机 OpenClaw、受限提醒代理、QQ 目标与验证步骤见 [OpenClaw QQ Bot 远程提醒配置指南](docs/OPENCLAW_QQ_REMINDERS.md)。
