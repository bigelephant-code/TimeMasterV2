...
const _sfc_main = {
  ...
  setup(__props) {
    const props = __props;
    const PATHS = {
      calendar: "M3 6a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v13a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6zM3 10h18M8 3v4M16 3v4",
      list: "M4 6h2M4 12h2M4 18h2M10 6h10M10 12h10M10 18h10",
      matrix: "M4 4h7v7H4zM13 4h7v7h-7zM4 13h7v7H4zM13 13h7v7h-7z",
      wallet: "M3 7a2 2 0 0 1 2-2h11a2 2 0 0 1 2 2v1M3 7v10a2 2 0 0 0 2 2h13a2 2 0 0 0 2-2v-2M3 7h15M21 10h-4a2 2 0 0 0 0 5h4v-5z",
      settings: "M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM19.4 15a1.6 1.6 0 0 0 .32 1.76l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.6 1.6 0 0 0-1.76-.32 1.6 1.6 0 0 0-1 1.46V21a2 2 0 1 1-4 0v-.1A1.6 1.6 0 0 0 9.1 19.4a1.6 1.6 0 0 0-1.76.32l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.6 1.6 0 0 0 .32-1.76 1.6 1.6 0 0 0-1.46-1H3a2 2 0 1 1 0-4h.1a1.6 1.6 0 0 0 1.46-1 1.6 1.6 0 0 0-.32-1.76l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.6 1.6 0 0 0 1.76.32H9a1.6 1.6 0 0 0 1-1.46V3a2 2 0 1 1 4 0v.1a1.6 1.6 0 0 0 1 1.46 1.6 1.6 0 0 0 1.76-.32l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.6 1.6 0 0 0-.32 1.76V9a1.6 1.6 0 0 0 1.46 1H21a2 2 0 1 1 0 4h-.1a1.6 1.6 0 0 0-1.5 1z",
      sun: "M12 17a5 5 0 1 0 0-10 5 5 0 0 0 0 10zM12 1v2M12 21v2M4.2 4.2l1.4 1.4M18.4 18.4l1.4 1.4M1 12h2M21 12h2M4.2 19.8l1.4-1.4M18.4 5.6l1.4-1.4",
      moon: "M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z",
      plus: "M12 5v14M5 12h14",
      check: "M20 6 9 17l-5-5",
      trash: "M3 6h18M8 6V4a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6",
      pencil: "M4 20h4l11-11a2.8 2.8 0 0 0-4-4L4 16v4zM13.5 6.5l4 4",
      ...
      cloud: "M7 19h10a4 4 0 0 0 .6-7.95A6 6 0 0 0 6.2 9.4 4.8 4.8 0 0 0 7 19z",
      "cloud-sun": "M8.4 7.2A4.5 4.5 0 0 1 17 9M16 2v2M21 7h-2M19.5 3.5 18 5M7 19h10a4 4 0 0 0 .6-7.95A6 6 0 0 0 6.2 9.4 4.8 4.8 0 0 0 7 19z",
      "cloud-rain": "M7 16h10a4 4 0 0 0 .6-7.95A6 6 0 0 0 6.2 6.4 4.8 4.8 0 0 0 7 16zM8 20l-1 2M13 20l-1 2M18 20l-1 2",
      "cloud-fog": "M7 15h10a4 4 0 0 0 .6-7.95A6 6 0 0 0 6.2 5.4 4.8 4.8 0 0 0 7 15zM5 19h14M8 22h8",
      "cloud-lightning": "M7 16h10a4 4 0 0 0 .6-7.95A6 6 0 0 0 6.2 6.4 4.8 4.8 0 0 0 7 16zM13 18l-2 3h3l-2 3",
      snowflake: "M12 2v20M4.2 6.5l15.6 11M19.8 6.5l-15.6 11M9 4l3 3 3-3M9 20l3-3 3 3M4 10l4 1-1-4M20 14l-4-1 1 4M20 10l-4 1 1-4M4 14l4-1-1 4",
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("svg", {
        ...
        createBaseVNode("path", { d: d.value }, null, 8, _hoisted_2)
      ], 8, _hoisted_1);
    };
  }
};
...
function parseYmd$1(str) {
  const [y, m, d] = String(str).split("-").map(Number);
  return new Date(y, m - 1, d);
}
const todayYmd$1 = () => ymd(/* @__PURE__ */ new Date());
function addDays(str, n) {
  const d = parseYmd$1(str);
...
function monthGrid(anchor, weekStart = 1) {
  const d = parseYmd$1(anchor);
...
function weekGrid(anchor, weekStart = 1) {
  const d = parseYmd$1(anchor);
...
function weekdayLabels(weekStart = 1) {
  const names = ["周日", "周一", "周二", "周三", "周四", "周五", "周六"];
  return Array.from({ length: 7 }, (_, i) => names[(i + weekStart) % 7]);
}
function isoWeekNumber(str) {
  const d = parseYmd$1(str);
  const target = new Date(d.getFullYear(), d.getMonth(), d.getDate());
  target.setDate(target.getDate() + 3 - (target.getDay() + 6) % 7);
  const firstThursday = new Date(target.getFullYear(), 0, 4);
  firstThursday.setDate(firstThursday.getDate() + 3 - (firstThursday.getDay() + 6) % 7);
  return 1 + Math.round((target - firstThursday) / (7 * 864e5));
}
function relativeLabel(str) {
  if (!str) return "";
  const diff = Math.round((parseYmd$1(str) - parseYmd$1(todayYmd$1())) / 864e5);
  if (diff === 0) return "今天";
  if (diff === 1) return "明天";
  if (diff === 2) return "后天";
  if (diff === -1) return "昨天";
...
function daysLeftInYear(str = todayYmd$1()) {
  const d = parseYmd$1(str);
  const last = new Date(d.getFullYear(), 11, 31);
  return Math.max(0, Math.round((last - d) / 864e5));
}
function daysBetween(from, to) {
  const a = parseYmd$1(from);
  const b = parseYmd$1(to);
  a.setHours(0, 0, 0, 0);
  b.setHours(0, 0, 0, 0);
...
function taskTimeLabel(todo) {
  ...
  if (start && end) return `开始 ${start} · 预计 ${taskDurationMinutes(todo)} 分钟完成`;
...
const PERIOD_IDS = GOAL_PERIODS.map((p2) => p2.id);
...
function periodKeyOf(period, at = Date.now()) {
  ...
  switch (normalizeGoalPeriod(period)) {
    case "day":
      return `${y}-${pad$1(m + 1)}-${pad$1(d.getDate())}`;
    case "week": {
      const { isoYear, week } = isoWeekParts(d);
      return `${isoYear}-W${pad$1(week)}`;
    }
    case "quarter":
...
function periodLabel(period, key) {
  const meta = GOAL_PERIODS.find((p2) => p2.id === normalizeGoalPeriod(period));
  const short = periodShortLabel(period, key);
  return short ? `${meta.current} · ${short}` : meta.current;
}
function periodShortLabel(period, key) {
  if (!key) return "";
  ...
  switch (normalizeGoalPeriod(period)) {
    ...
    case "week":
      return `第${Number(String(parts[1]).replace("W", ""))}周`;
    case "quarter":
    case "half":
      return parts[1] || "";
    case "year":
...
const todayYmd = () => ymdOf(/* @__PURE__ */ new Date());
function addYmdDays(str, n) {
  const d = parseYmd(str);
  d.setDate(d.getDate() + n);
  return ymdOf(d);
}
function addYmdMonths(str, n) {
  const d = parseYmd(str);
  const day = d.getDate();
  d.setDate(1);
  d.setMonth(d.getMonth() + n);
...
const periodKeyOfYmd = (period, str) => periodKeyOf(period, parseYmd(str));
function stepPeriod(period, str, n) {
  switch (normalizeGoalPeriod(period)) {
    case "day":
      return addYmdDays(str, n);
    case "week":
      return addYmdDays(str, n * 7);
    case "quarter":
      return addYmdMonths(str, n * 3);
    case "half":
      return addYmdMonths(str, n * 6);
    case "year":
      return addYmdMonths(str, n * 12);
    default:
      return addYmdMonths(str, n);
  }
}
function periodBounds(period, str = todayYmd()) {
  const d = parseYmd(str);
  const y = d.getFullYear();
  const m = d.getMonth();
  const last = (yy, mm) => ymdOf(new Date(yy, mm + 1, 0));
  switch (normalizeGoalPeriod(period)) {
    case "day":
      return [str, str];
    case "week": {
      const monday = addYmdDays(str, -((d.getDay() + 6) % 7));
      return [monday, addYmdDays(monday, 6)];
    }
    case "quarter": {
      const q = Math.floor(m / 3) * 3;
      return [`${y}-${pad(q + 1)}-01`, last(y, q + 2)];
    }
    case "half": {
      const h = m < 6 ? 0 : 6;
      return [`${y}-${pad(h + 1)}-01`, last(y, h + 5)];
    }
    case "year":
      return [`${y}-01-01`, `${y}-12-31`];
    default:
      return [`${y}-${pad(m + 1)}-01`, last(y, m)];
  }
}
function recentPeriodKeys(period, count, anchor = todayYmd()) {
  const keys = [];
  let cursor = anchor;
  for (let i = 0; i < count; i++) {
    keys.unshift(periodKeyOfYmd(period, cursor));
    cursor = stepPeriod(period, cursor, -1);
  }
  return keys;
}
...
function groupByDate(entries = []) {
  ...
  for (const e of entries) {
    const date = normalizeYmd(e.date);
    if (!date) continue;
...
function groupByPeriod(entries = [], period = "month") {
  const map = /* @__PURE__ */ new Map();
  for (const e of entries) {
    const date = normalizeYmd(e.date);
    if (!date) continue;
    const key = periodKeyOfYmd(period, date);
...
function opexRanking(totals, { includeZero = false, goal = null } = {}) {
  const rows = opexCatsOf(goal).map((c) => ({ ...c, amount: totals.byCat[c.id] || 0 })).filter((r) => includeZero || r.amount !== 0);
  const max = Math.max(1, ...rows.map((r) => Math.abs(r.amount)));
  const sum = rows.reduce((acc, r) => acc + r.amount, 0);
  return rows.sort((a, b) => Math.abs(b.amount) - Math.abs(a.amount)).map((r) => ({
    ...r,
    // 0 就是不画：留一截色块会被读成"有一点点"，跟右边的 0 自相矛盾。
    // 非 0 的给 2% 下限，金额很小的类别也得留一截看得见的颜色
    width: r.amount === 0 ? 0 : Math.max(2, Math.round(Math.abs(r.amount) / max * 100)),
    share: sum ? Math.round(r.amount / sum * 100) : 0
  }));
}
function compareWithPrev(current, prev) {
  const diff = round2((Number(current) || 0) - (Number(prev) || 0));
...
const state = /* @__PURE__ */ reactive({
  ...
  expenseGoalId: null,
  // 费用视图当前台账
  expenseOpenRequest: null,
  // 小组件定向打开费用明细；带序号以便同一天重复触发
  activeListId: null,
  // null = 全部清单
  filter: "all",
  // all | active | done
...
const listById = computed(() => new Map(state.lists.map((l) => [l.id, l])));
const ledgerGoals = computed(() => state.goals.filter((g) => g.mode === "ledger"));
function expensesOfGoal(goalId) {
  return goalId ? state.expenses.filter((e) => e.goalId === goalId) : [];
}
const expensesByDate = computed(() => groupByDate(state.expenses));
function expensesOn(dateStr) {
  return expensesByDate.value.get(dateStr) || [];
}
...
const actions = {
  ...
  addExpense: (input) => api.expenses.add(input),
  updateExpense: (id, patch) => api.expenses.update(id, patch),
...
const needsSeconds = computed(
  () => state.focusTimer?.status === "running" || state.todos.some((t) => t.startedAt && !t.done)
);
let handle = null;
function tick() {
  now.value = Date.now();
  const period = needsSeconds.value ? 1e3 : 6e4;
  handle = setTimeout(tick, period - Date.now() % period + 20);
}
watch(needsSeconds, () => {
  clearTimeout(handle);
  tick();
...
function dueAtOf(todo) {
  if (!todo.date) return null;
  const deadlineTime = taskStartTime(todo) || taskEndTime(todo);
  if (!deadlineTime) return null;
  const d = parseYmd$1(todo.date);
  const [h, m] = deadlineTime.split(":").map(Number);
  d.setHours(h, m, 0, 0);
  return d.getTime();
}
function remainingMinutes(todo, now2 = Date.now()) {
  const due = dueAtOf(todo);
  if (due === null) return null;
  return Math.floor((due - now2) / 6e4);
}
function deadlineState(todo, now2 = Date.now()) {
  const left = remainingMinutes(todo, now2);
  if (left === null) return null;
  if (left < 0) return "overdue";
  if (left <= URGENT_MINUTES) return "urgent";
  return null;
}
function elapsedMsOf(todo, now2 = Date.now()) {
  const base = todo.elapsedMs || 0;
  return todo.startedAt ? base + (now2 - todo.startedAt) : base;
}
function todoState(todo, now2 = Date.now()) {
  if (todo.done) return "done";
  if (todo.startedAt) return "running";
  return deadlineState(todo, now2) || "normal";
}
function timerDeadlineState(todo, now2 = Date.now()) {
  if (todo.done || !todo.startedAt) return null;
  return deadlineState(todo, now2);
}
...
function durationLabel(ms) {
  ...
  if (ms < 6e4) return "不足 1 分钟";
  return `${Math.round(ms / 6e4)} 分钟`;
}
function remainingLabel(todo, now2 = Date.now()) {
  const left = remainingMinutes(todo, now2);
  if (left === null) return null;
  if (left < 0) {
    const over = -left;
...
var lunar$1 = { exports: {} };
var lunar = lunar$1.exports;
var hasRequiredLunar;
function requireLunar() {
  if (hasRequiredLunar) return lunar$1.exports;
  hasRequiredLunar = 1;
  (function(module) {
    (function(root, factory) {
      ...
      var Lunar = /* @__PURE__ */ (function() {
        ...
        var _computeMonth = function(o, solar) {
          ...
          for (i = 0; i < size; i += 2) {
            ...
            var ymd2 = solar.toYmd();
            var symd = null == start ? ymd2 : start.toYmd();
            if (ymd2 >= symd && ymd2 < end.toYmd()) {
              break;
            }
            start = end;
            index++;
      ...
      var LunarTime = /* @__PURE__ */ (function() {
        var _fromYmdHms = function(lunarYear, lunarMonth, lunarDay, hour, minute, second) {
          var lunar2 = Lunar.fromYmdHms(lunarYear, lunarMonth, lunarDay, hour, minute, second);
  ...
  return lunar$1.exports;
}
var lunarJavascript;
var hasRequiredLunarJavascript;
function requireLunarJavascript() {
  if (hasRequiredLunarJavascript) return lunarJavascript;
  hasRequiredLunarJavascript = 1;
  const { Solar, Lunar, Foto, Tao, NineStar, EightChar, SolarWeek, SolarMonth, SolarSeason, SolarHalfYear, SolarYear, LunarMonth, LunarYear, LunarTime, ShouXingUtil, SolarUtil, LunarUtil, FotoUtil, TaoUtil, HolidayUtil, NineStarUtil, I18n } = requireLunar();
  lunarJavascript = {
    Solar,
    Lunar,
    Foto,
  ...
  return lunarJavascript;
}
var lunarJavascriptExports = requireLunarJavascript();
const cache = /* @__PURE__ */ new Map();
function compute(dateStr) {
  const d = parseYmd$1(dateStr);
  const solar = lunarJavascriptExports.Solar.fromYmd(d.getFullYear(), d.getMonth() + 1, d.getDate());
  const lunar2 = solar.getLunar();
  const jieQi = lunar2.getJieQi() || "";
  const lunarFestivals = lunar2.getFestivals() || [];
  const solarFestivals = solar.getFestivals() || [];
  const dayInChinese = lunar2.getDayInChinese();
  const monthInChinese = `${lunar2.getMonthInChinese()}月`;
  let holiday = null;
  try {
    const h = lunarJavascriptExports.HolidayUtil.getHoliday(d.getFullYear(), d.getMonth() + 1, d.getDate());
  ...
  return {
    ...
    holiday,
    ganzhi: `${lunar2.getYearInGanZhi()}年 ${lunar2.getMonthInGanZhi()}月 ${lunar2.getDayInGanZhi()}日`,
    animal: lunar2.getYearShengXiao(),
    /** 紧凑写法，给小组件那一行用：丙午马年 */
    yearGz: `${lunar2.getYearInGanZhi()}${lunar2.getYearShengXiao()}年`,
    /** 完整农历表述，小组件和日详情用 */
    full: `${lunar2.getMonthInChinese()}月${dayInChinese}`
  };
}
function lunarInfo(dateStr) {
...
export {
  ...
  addYmdDays as aI,
  MAX_CAT_NAME as aa,
  periodShortLabel as ab,
  periodBounds as ac,
  expensesOfGoal as ad,
  ymdOf as ae,
  periodKeyOfYmd as af,
  compareWithPrev as ag,
  opexRanking as ah,
  periodSeries as ai,
  groupByDate as aj,
  groupByPeriod as ak,
  ...
  vModelSelect as am,
  REPEATS as an,
  REMIND_OPTIONS as ao,
  taskDurationMinutes as ap,
